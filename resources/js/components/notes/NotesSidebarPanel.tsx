...
const Section = ({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <span className="block px-0.5 text-xsmall font-bold tracking-wide text-dark-primary/60 uppercase">{label}</span>
    ...
    <section className="flex min-h-0 w-[401px] shrink-0 flex-col overflow-hidden rounded-lg border border-dark-border/10 bg-dark-surface-2">
      <div className="px-4 pt-4">
        ...
        <div className="mb-4 flex h-9 items-center rounded-lg border border-dark-border/40 bg-dark-surface-3">
          ...
          />
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mr-3 shrink-0 text-dark-secondary">
      ...
      <div className="flex flex-1 flex-col gap-4 overflow-y-auto px-4 pb-4 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
        {filtered.length === 0 ? (
          <p className="mt-4 text-center text-small text-dark-secondary">No notes found.</p>
        ) : (
          <>
            <Section label="Private" notes={privateNotes} selectedNoteId={selectedNoteId} onSelectNote={onSelectNote} onDeleteRequest={onDeleteRequest} />
            <Section label="Shared" notes={sharedNotes} selectedNoteId={selectedNoteId} onSelectNote={onSelectNote} onDeleteRequest={onDeleteRequest} />
          </>
        )}
      </div>
    </section>
