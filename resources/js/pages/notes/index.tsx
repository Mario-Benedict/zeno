...
const NotesPage = ({ notes: initialNotes, projectUsers, currentUserId }: NotesPageProps): React.ReactElement => {
  ...
  const openNote = useCallback(
    async (item: NoteListItem) => {
      const { data } = await axios.get<{ note: NoteDetail }>(notes.show.url({ project: projectSlug, note: item.id }));
  ...
  const handleCreate = useCallback(
    async (isShared: boolean) => {
      const { data } = await axios.post<{ note: NoteDetail }>(notes.store.url({ project: projectSlug }), {
        title: 'Untitled',
        is_shared: isShared,
      });
    ...
    [projectSlug, upsertListItem],
  );

  const handleDelete = useCallback(async () => {
    if (!deleteTargetId) return;

    await axios.delete(notes.destroy.url({ project: projectSlug, note: deleteTargetId }));
    setNoteList((prev) => prev.filter((n) => n.id !== deleteTargetId));
    setSelectedNote((prev) => (prev?.id === deleteTargetId ? null : prev));
    setDeleteTargetId(null);
  }, [projectSlug, deleteTargetId]);
