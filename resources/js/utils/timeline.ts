  ...
  boards.forEach((board, boardIndex) => {
    (board.cards ?? []).forEach((card) => {
      ...
      tasks.push({
        ...
        title: detail.kanban_board_card_title,
        isCompleted: detail.is_completed,
        start: parseCardDate(detail.dates?.kanban_board_card_start_date),
        due: parseCardDate(detail.dates?.kanban_board_card_due_date),
...
export const getPriorityRank = (task: TimelineTask): number => {
  ...
  task.labels.forEach((label) => {
    ...
    const colorRank = label.color?.card_label_color_hex
      ? (COLOR_WARMTH_RANK[label.color.card_label_color_hex.toUpperCase()] ?? 0)
      : 0;
...
export const getTaskAccentColor = (task: TimelineTask): string =>
  task.labels.find((l) => l.color?.card_label_color_hex)?.color
    ?.card_label_color_hex ?? '#3949AB';
