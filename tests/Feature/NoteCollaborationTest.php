...
class NoteCollaborationTest extends TestCase
{
    ...
    public function non_project_members_are_forbidden_from_viewing_the_workspace()
    {
        ...
        $this->actingAs($unauthorizedUser)
            ->get("/u/0/p/{$project->project_slug}/notes")
            ->assertStatus(403);
    }
    ...
    public function authorized_members_can_create_shared_notes()
    {
        ...
        $response = $this->actingAs($user)
            ->postJson("/u/0/p/{$project->project_slug}/notes", [
    ...
    public function owner_can_add_a_project_member_as_a_collaborator()
    {
        ...
        $this->actingAs($owner)
            ->postJson("/u/0/p/{$project->project_slug}/notes/{$note->note_id}/collaborators", [
                'user_id' => $member->id,
                'can_edit' => true,
            ])
            ->assertStatus(200);
    ...
    public function a_viewer_collaborator_cannot_edit_but_an_editor_can()
    {
        ...
        $this->actingAs($viewer)
            ->patchJson("/u/0/p/{$project->project_slug}/notes/{$note->note_id}", ['title' => 'Nope'])
            ->assertStatus(403);

        $this->actingAs($editor)
            ->patchJson("/u/0/p/{$project->project_slug}/notes/{$note->note_id}", ['title' => 'Updated'])
            ->assertStatus(200);
    }
    ...
    public function can_upload_an_image_into_a_note()
    {
        ...
        $response = $this->actingAs($owner)
            ->postJson("/u/0/p/{$project->project_slug}/notes/{$note->note_id}/images", [
