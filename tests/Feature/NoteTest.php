...
it('loads the unified notes workspace with a lightweight note list', function () {
    ...
    $this->actingAs($this->user1)
        ->get("/u/0/p/{$this->project->project_slug}/notes")
        ->assertStatus(200)
...
it('can create a new personal note with default state', function () {
    /** @var mixed $this */
    $this->actingAs($this->user1)
        ->postJson("/u/0/p/{$this->project->project_slug}/notes", [
            'title' => 'Untitled',
            'is_shared' => false,
        ])
        ->assertStatus(201)
...
it('fetches full note content on demand via the show endpoint', function () {
    ...
    $this->actingAs($this->user1)
        ->getJson("/u/0/p/{$this->project->project_slug}/notes/{$note->note_id}")
        ->assertStatus(200)
...
it('computes and stores a plain-text excerpt on update', function () {
    ...
    $this->actingAs($this->user1)
        ->patchJson("/u/0/p/{$this->project->project_slug}/notes/{$note->note_id}", [
            'title' => 'Doc',
...
it('can soft delete a note from the workspace', function () {
    ...
    $this->actingAs($this->user1)
        ->deleteJson("/u/0/p/{$this->project->project_slug}/notes/{$note->note_id}")
        ->assertStatus(200)
...
it('lets the owner promote a personal note to shared and seed collaborators', function () {
    ...
    $this->actingAs($this->user1)
        ->postJson("/u/0/p/{$this->project->project_slug}/notes/{$note->note_id}/share", [
            'collaborators' => [
