...
/**
 * Seed a scheduled task (board → card) and return the card.
 */
...
it('exposes each scheduled task\'s start and due dates for the bars', function () {
    ...
    $this->actingAs($this->user1)
        ...
        ->assertInertia(fn ($page) => $page
            ->component('timeline')
            ->where('kanbanBoards.0.cards.0.kanban_board_card_start_date', fn ($d) => str_contains((string) $d, '2026-04-27'))
            ->where('kanbanBoards.0.cards.0.kanban_board_card_due_date', fn ($d) => str_contains((string) $d, '2026-04-30'))
        );
});
